...
run_test = function(Sim_name = 'Simulation', 
                    nchains = 1, niter = 210000, nthin = 10,
                    nburn = 10000, K = c(1,2,5,10,15,20), N=5,
                    save.every = length(K), add_AF_slice = TRUE){
  ...
  # K (vector:int): number of data clones at every iteration
  ...
  SimParams1 <- SimParams # initialise parameters
  ...
  for (i in 1:N){
    for (k in 1:length(K)){
      ...
      print(paste('Data set', as.character(i))) # give update on data set
      print(paste('Cloning', as.character(k))) # give update on position
      
      data1 <- data_list[[i]] # data set for this cloning chunk
      K_current <- K[k]
      
      ############################### clone the data
      data$Occ <- do.call(rbind, replicate(K_current, data1$Occ, simplify=FALSE))
      data$site.occ <- rep(data1$site.occ, K_current)
      data$det_alpha <- do.call(abind, list(replicate(K_current, data1$det_alpha, simplify=FALSE),
                                            'along' = 1))
      data$DTD1 <- do.call(rbind, replicate(K_current, data1$DTD1, simplify=FALSE))
      data$DND1 <- do.call(rbind, replicate(K_current, data1$DND1, simplify=FALSE))
      data$DND2 <- do.call(abind, list(replicate(K_current, data1$DND2, simplify=FALSE),
                                        'along' = 1))
      data$DTD2 <- do.call(abind, list(replicate(K_current, data1$DTD2, simplify=FALSE),
                                        'along' = 1))
      data$C1 <- do.call(rbind, replicate(K_current, data1$C1, simplify=FALSE))
      
      ##########################
      
      # update SimParams
      SimParams$num_sites <- K_current * SimParams1$num_sites
      ...
      ################################## Run the model
      mcmcConf1d = configureMCMC(DEmodel1d, monitors = c('p', 'theta01', 'theta11', 'psi'
      ), print = FALSE, enableWAIC = TRUE)
      mcmcConf1c = configureMCMC(DEmodel1c, monitors = c('p', 'theta01', 'theta11', 'psi'
      ), print = FALSE, enableWAIC = TRUE)
      mcmcConf2d = configureMCMC(DEmodel2d, monitors = c('p', 'theta01', 'theta11', 'psi'
      ), print = FALSE, enableWAIC = TRUE)
      mcmcConf2c = configureMCMC(DEmodel2c, monitors = c('p', 'theta01', 'theta11', 'psi'
      ), print = FALSE, enableWAIC = TRUE)
      mcmcConf4d = configureMCMC(DEmodel4d, monitors = c('p', 'theta01', 'theta11', 'psi'
      ), print = FALSE, enableWAIC = TRUE)
      ...
      if (k == 1){
        ...
        if (nchains == 1){
          ...
          par_names1d = colnames(DEresults1d$samples$chain1)
          par_names1c = colnames(DEresults1c$samples$chain1)
          par_names2d = colnames(DEresults2d$samples$chain1)
          par_names2c = colnames(DEresults2c$samples$chain1)
          par_names4d = colnames(DEresults4d$samples$chain1)
          nsaved.iters <- nchains*nrow(DEresults1d$samples$chain1)
        }
        
        for (par in par_names1d){
          par_chain1d[[par]] = matrix(numeric(0), nrow = nsaved.iters, ncol = length(K))
        }
        for (par in par_names1c){
          par_chain1c[[par]] = matrix(numeric(0), nrow = nsaved.iters, ncol = length(K))
        }
        for (par in par_names2d){
          par_chain2d[[par]] = matrix(numeric(0), nrow = nsaved.iters, ncol = length(K))
        }
        for (par in par_names2c){
          par_chain2c[[par]] = matrix(numeric(0), nrow = nsaved.iters, ncol = length(K))
        }
        for (par in par_names4d){
          par_chain4d[[par]] = matrix(numeric(0), nrow = nsaved.iters, ncol = length(K))
        }
      }
      
      if (nchains == 1){
        for (par in par_names1d){
          par_chain1d[[par]][,k] = DEresults1d$samples[,par]
        }
        for (par in par_names1c){
          par_chain1c[[par]][,k] = DEresults1c$samples[,par]
        }
        for (par in par_names2d){
          par_chain2d[[par]][,k] = DEresults2d$samples[,par]
        }
        for (par in par_names2c){
          par_chain2c[[par]][,k] = DEresults2c$samples[,par]
        }
        for (par in par_names4d){
          par_chain4d[[par]][,k] = DEresults4d$samples[,par]
        }
      } else {
        for (par in par_names1d){
          par_chain1d[[par]][,k] = do.call(rbind, DEresults1d$samples)[,par]
        }
        for (par in par_names1c){
          par_chain1c[[par]][,k] = do.call(rbind, DEresults1c$samples)[,par]
        }
        for (par in par_names2d){
          par_chain2d[[par]][,k] = do.call(rbind, DEresults2d$samples)[,par]
        }
        for (par in par_names2c){
          par_chain2c[[par]][,k] = do.call(rbind, DEresults2c$samples)[,par]
        }
        for (par in par_names4d){
          par_chain4d[[par]][,k] = do.call(rbind, DEresults4d$samples)[,par]
        }
      }
      
      if (k%%save.every == 0){
        filename <- paste0(Sim_name, '_N', as.character(i), '.RData')
