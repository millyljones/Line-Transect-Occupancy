...
Delta_metric <- function(data, num_trans){
  # data is the data set. 
  # data needs a site.occ list
  ...
  if (J.max == 1){
    ...
    zones_to_trans <- rep(1:num_zones, each = num_trans)
...
# Sim data
# Need to think about whether we want to fix the number of segments
# or fix the transect length in order to get the point across.

# state 2 is the occupied state
...
# Will have 9 parameter settings, ranging from low use, medium use, high use, each at low det, medium det, high det.
Pars <- as.character(1:9)
lambda <- rep(c(1/300, 1/700, 1/1200), each = 3)
mu12 <- rep(c(1/300, 1/700, 1/1200), 3); mu21 <- rep(c(1/300, 1/700, 1/1200), 3) # state 2 is occupied
init <- mu12/(mu21+mu12) # state 2 is occupied

R.vals <- c(seq(10, 500, length.out = 50), seq(500, 2500, length.out = 100))

df <- data.frame(matrix(ncol = 6, nrow=0))
colnames(df) <- c('Pars', 'lambda', 'mu12', 'mu21', 'Delta', 'R')
...
for (i in 1:length(Pars)){
  ...
  for (R in R.vals){
    num_segments <- L%/%R
    data <- simulate_data_continuous(num_sites, num_trans, num_segments, psi, init[i], 
                                     mu12[i], mu21[i], lambda[i], R)
    Delta.vals <- c(Delta.vals, Delta_metric(data, num_trans))
  }
  df <- rbind(df, data.frame('Pars'=Pars[i],
                             'lambda'=lambda[i],
                             'mu12'=mu12[i],
                             'mu21'=mu21[i],
                             'Delta'=Delta.vals,
...
save(df, file = 'R_lengths.RData')

library(ggplot2)

df$switching <- c('S: high', 'S: med', 'S: low')[sapply(df$mu12, function(x){which(x==c(1/300, 1/700, 1/1200))})]
df$detection <- c('D: high', 'D: med', 'D: low')[sapply(df$lambda, function(x){which(x==c(1/300, 1/700, 1/1200))})]

df$switching <- factor(df$switching, levels = c('S: low', 'S: med', 'S: high'))
df$detection <- factor(df$detection, levels = c('D: low', 'D: med', 'D: high'))

df$Setting <- paste0(df$switching, '. ', df$detection)
df$Setting <- factor(df$Setting, levels = c('S: low. D: low',
                                               'S: med. D: low',
                                               'S: high. D: low',
                                               'S: low. D: med',
                                               'S: med. D: med',
...
ggplot(df, aes(x=R, y=Delta,colour=Setting))+
  facet_grid(detection~switching)+
  geom_point()+
  geom_hline(yintercept=0)+
  ylab(expression(Delta))+
  theme_bw()+
  theme(legend.position = 'none')+
  scale_x_continuous(breaks=c(0,500,1500,2500))

ggsave('R_lengths.png', width=6, height=6, units='in')



# Can do the same experiment as above but change the 
